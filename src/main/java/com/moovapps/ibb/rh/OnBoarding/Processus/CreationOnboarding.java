...
public class CreationOnboarding extends BaseDocumentExtension {
    ...
    public boolean onBeforeSubmit(IAction action) {
        if(action.getName().equals("LancerLOnboardingDuCollaborateur")){
            ...
            createEtapeOnboarding();

        }
        return super.onBeforeSubmit(action);
    }

    private void createEtapeOnboarding() {
        ArrayList<IUser> responsablesTraitement = new ArrayList<>();
        ArrayList<ILinkedResource> parcoursDIntegration = (ArrayList<ILinkedResource>) getWorkflowInstance().getValue("ParcoursDIntegrationMetier");
        parcoursDIntegration.addAll((ArrayList<ILinkedResource>) getWorkflowInstance().getValue("ParcoursDIntegrationAdministratif"));
        for (ILinkedResource parcour : parcoursDIntegration) {
            try {
                IWorkflow etapeOnBoardingWorkflow = getWorkflowModule().getWorkflow(getWorkflowModule().getSysadminContext(), getWorkflowInstance().getCatalog(), "EtapeOnBoarding_1.0");
                IWorkflowInstance etapeOnBoarding = getWorkflowModule().createWorkflowInstance(getWorkflowModule().getSysadminContext(), etapeOnBoardingWorkflow, "");
                etapeOnBoarding.setValue("Etape",parcour.getValue("Etape"));
                etapeOnBoarding.setValue("Onboardeur2",parcour.getValue("Interlocuteur"));
                etapeOnBoarding.setValue("Collaborateur", getWorkflowInstance().getValue("Collaborateur"));
                etapeOnBoarding.setValue("TypeDeParcours",parcour.getValue("TypeDeParcours"));
                etapeOnBoarding.save(getWorkflowModule().getSysadminContext());
                getWorkflowInstance().addLinkedWorkflowInstance("EtapeOnBoarding",etapeOnBoarding);
                ...
                passerALEtapeSuivante(etapeOnBoarding);
    ...
    ArrayList<IStorageResource> getParcoursAdministratif(IStorageResource categorie){
        try {
            ...
            IResourceDefinition definition = getWorkflowModule().getResourceDefinition(context, catalog, "ParcoursDIntegrationAdministratif");
    ...
    ArrayList<IStorageResource> getParcoursMetier(IStorageResource categorie){
        try {
            ...
            IProject project = getProjectModule().getProject(context, "OnBoardingOffBoarding", organization);
            ICatalog catalog = getWorkflowModule().getCatalog(context, "ReferentielOnBoardingOffBoarding",ICatalog.IType.STORAGE, project);
            IResourceDefinition definition = getWorkflowModule().getResourceDefinition(context, catalog, "ParcoursDIntegration");
            controller.addEqualsConstraint("Categorie", categorie);
            return (ArrayList<IStorageResource>) controller.evaluate(definition);
        } catch (Exception e) {
            e.printStackTrace();
