...
public class ModuleSortieDash extends BaseController {
	
	protected static final Logger log = Logger.getLogger(ModuleSortieDash.class);
	...
	private JSONObject BuildData() {
		...
		try
		{
			...
			listFiltres.add("Date déclaration");
			...
			Collection<IWorkflowInstance> sorties =  getData();
			SimpleDateFormat monthYearFormat = new SimpleDateFormat("MM-yy");
				for (IWorkflowInstance sortie : sorties)
				{
					...
					tmp.put("Demandeur", ((IUser)sortie.getValue("Demandeur")).getFullName());
					tmp.put("Date déclaration", monthYearFormat.format((Date)sortie.getValue("dateSortie")));
					tmp.put("Durée", new DateHelper().durationToDays(((Number)sortie.getValue("dureeSortieMinutesReelle")).longValue(), societe));
