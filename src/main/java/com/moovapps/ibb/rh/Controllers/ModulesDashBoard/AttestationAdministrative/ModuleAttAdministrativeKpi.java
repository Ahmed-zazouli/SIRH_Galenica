package com.moovapps.ibb.rh.Controllers.ModulesDashBoard.AttestationAdministrative;
...
public class ModuleAttAdministrativeKpi extends BaseController {

	protected static final Logger log = Logger.getLogger(ModuleAttAdministrativeKpi.class);
	...
	private JSONObject BuildData() {
		...
		try {
			Collection<IWorkflowInstance> attsAdministratives = getData();
			int demandesEnCours = 0;
			int demandesARecuperee = 0;
			int demandesTraitess = 0;
			IStorageResource societe = getSociete((String)connectedUser.getExtendedAttributes().getValue("Societe"));
			for (IWorkflowInstance attestation : attsAdministratives) {
				if(attestation.getValue("DocumentState").equals("Clôturée")){
					demandesTraitess += 1;
				} else if(attestation.getValue("DocumentState").equals("Attestation générée")){
					demandesARecuperee += 1;
				} 
				if(!attestation.getValue("DocumentState").equals("Clôturée")){
					demandesEnCours += 1;
				}
			...
			tmp.put("kpi1_value", demandesEnCours);
			tmp.put("kpi2_value", demandesARecuperee);
			tmp.put("kpi3_value", demandesTraitess);
