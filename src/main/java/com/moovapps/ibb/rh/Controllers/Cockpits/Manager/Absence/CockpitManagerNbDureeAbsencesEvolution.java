...
public class CockpitManagerNbDureeAbsencesEvolution extends BaseController {
	...
	protected static final Logger log = Logger.getLogger(CockpitManagerNbDureeAbsencesEvolution.class);
	...
	private JSONObject BuildData() {
		...
		{	
			...
			for (IWorkflowInstance conge : demandesConges)
			{
				...
				String demandeur = conge.getValue("Demandeur")!=null?((IUser) conge.getValue("Demandeur")).getFullName():"N/A";
				...
				GroupModel testObject = new GroupModel(demandeur, "1");
				if(!evolutionMap.containsKey(testObject)){
					for (int i = 1; i < 13; i++) {
						JSONObject tmp = new JSONObject();
						tmp.put("Nb Absences", 0);
						tmp.put("Durée Absences", 0);
						tmp.put("Salarié", conge.getValue("Demandeur") != null ? ((IUser)conge.getValue("Demandeur")).getFullName() : "N/D");
						tmp.put("Mois", String.valueOf(i));
						GroupModel tmpGroupModel = new GroupModel(demandeur, String.valueOf(i));
				...
				GroupModel tmpGroupModel = new GroupModel(demandeur, mois);
