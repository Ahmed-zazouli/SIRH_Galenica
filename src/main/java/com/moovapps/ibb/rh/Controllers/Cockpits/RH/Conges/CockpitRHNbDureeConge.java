...
public class CockpitRHNbDureeConge extends BaseController {
	
	protected static final Logger log = Logger.getLogger(CockpitRHNbDureeConge.class);
				...
				for (IWorkflowInstance conge : demandesConges)
				{
					...
					if(conge.getValue("TypeDeConge") != null){
						if(conge.getValue("TypeDeConge").equals("CE")){
							tmp.put("Durée congés", conge.getValue("NombreDeJoursExceptionnelle"));
						}else{
							tmp.put("Durée congés", conge.getValue("NombreDeJoursDemandes"));
						}
					}
