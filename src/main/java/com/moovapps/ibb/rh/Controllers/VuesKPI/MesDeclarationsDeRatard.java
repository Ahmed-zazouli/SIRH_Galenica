...
public class MesDeclarationsDeRatard extends BaseController {
	
	protected static final Logger log = Logger.getLogger(MesDeclarationsDeRatard.class);
...
//			tmp.put("kpi2_value", df.format(calculateJoursMoyenRetard(premiumMembers)));
			Collection<IWorkflowInstance> mesRetards = getData();
			float dureeRetardDuMois = 0;
			float dureeRetardTotal = 0;
			...
			for (IWorkflowInstance retard : mesRetards) {
				if(retard.getValue("dureeRetardReelleEnMinutes") != null){					
					if(((Number)retard.getValue("AnneeDuRetard")).intValue() == c.get(Calendar.YEAR) && ((Number)retard.getValue("MoisDuRetard")).intValue() == (c.get(Calendar.MONTH) + 1)){
						dureeRetardDuMois += ((Number)retard.getValue("dureeRetardReelleEnMinutes")).floatValue();
					}
					dureeRetardTotal += ((Number)retard.getValue("dureeRetardReelleEnMinutes")).floatValue();
				}
			}
			
			tmp.put("Svalue1", mesRetards.size());
			tmp.put("Svalue2", fromMinutesToDaysHoursMinutes(dureeRetardDuMois, societe));
			tmp.put("Svalue3", fromMinutesToDaysHoursMinutes(dureeRetardTotal, societe)); 
