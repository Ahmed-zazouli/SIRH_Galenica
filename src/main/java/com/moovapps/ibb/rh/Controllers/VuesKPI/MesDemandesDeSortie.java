...
public class MesDemandesDeSortie extends BaseController {
	
	protected static final Logger log = Logger.getLogger(MesDemandesDeSortie.class);
	...
	private JSONObject BuildData() {
		...
		try
		{
			...
			Collection<IWorkflowInstance> mesSorties = getData();
			float dureeSortiesDuMois = 0;
			float dureeSortiesTotal = 0;
			
			IStorageResource societe = (IStorageResource)getWorkflowModule().getLoggedOnUser().getExtendedAttributes().getValue("SocieteDonnee");
			
			Calendar c = Calendar.getInstance();
		
			for (IWorkflowInstance sortie : mesSorties) {
				if(sortie.getValue("dureeSortieMinutesReelle") != null){					
					if(((Number)sortie.getValue("AnneeSortie")).intValue() == c.get(Calendar.YEAR) && ((Number)sortie.getValue("MoisSortie")).intValue() == (c.get(Calendar.MONTH) + 1)){
						dureeSortiesDuMois += ((Number)sortie.getValue("dureeSortieMinutesReelle")).floatValue();
					}
					dureeSortiesTotal += ((Number)sortie.getValue("dureeSortieMinutesReelle")).floatValue();
				}
			}
			
			tmp.put("Svalue1", mesSorties.size());
			tmp.put("Svalue2", fromMinutesToDaysHoursMinutes(dureeSortiesDuMois, societe));
			tmp.put("Svalue3", fromMinutesToDaysHoursMinutes(dureeSortiesTotal, societe)); 
