...
public class CockpitRhKpiAbsence extends BaseController {

	protected static final Logger log = Logger
			.getLogger(CockpitRhKpiAbsence.class);
	...
	private JSONObject BuildData() {
		...
		try {
			Collection<IWorkflowInstance> todos = getTodo();
	...
	private Collection<IWorkflowInstance> getAllDemandeDeclares() {
		...
		try {
			...
			//IWorkflow w = getWorkflowModule().getWorkflow(sysContext, catalog, "GestionDeConges_1.0");
			IViewController controller = getWorkflowModule().getViewController(sysContext);
			controller.addEqualsConstraint("sys_WorkflowContainer", "GestionDeConges" );
...
/*	private Collection<IWorkflowInstance> getTodo() {
		Collection<IWorkflowInstance> collection = Collections.emptyList();
		try {
			IContext sysContext = getWorkflowModule().getSysadminContext();
			IUser connectedUser = getWorkflowModule().getLoggedOnUser();
			...
			collection = controller.evaluate(catalog);
			Collection<IWorkflowInstance> deleteCollection = new ArrayList<>();
			for (IWorkflowInstance instance : collection) {
				String names =(String) instance.getValue("sys_CurrentActors");
				String user = getWorkflowModule().getLoggedOnUser().getFullName();
				boolean nameExists = Arrays.stream(names.split("\\n"))
						.map(String::trim) // Remove leading/trailing spaces
						.collect(Collectors.toList())
						.contains(user);
				if (!nameExists) {
					deleteCollection.add(instance);
				}
			}
			collection.removeAll(deleteCollection);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return collection;
	...
	private Collection<IWorkflowInstance> getAllDemandesAbsencesCemMois() {
		...
		try {
			...
			IViewController controller = getWorkflowModule().getViewController(sysContext);
			controller.addEqualsConstraint("sys_WorkflowContainer", "GestionDeConges" );
			//controller.addInConstraint("sys_WorkflowContainer", Arrays.asList("EXECUTIONDESCONTROLES", "INSTANCESREMONTEESCONFORMITE", "MISSIONTHEMATIQUE"));
			controller.addEqualsConstraint("Societe", connectedUser.getExtendedAttributes().getValue("Societe"));
			controller.addNotEqualsConstraint("DocumentState", "En cours");
			controller.addEqualsConstraint("TypeDeConge", "Absence");
