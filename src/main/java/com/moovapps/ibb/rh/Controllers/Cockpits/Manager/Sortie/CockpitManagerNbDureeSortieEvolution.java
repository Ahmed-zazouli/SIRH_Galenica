package com.moovapps.ibb.rh.Controllers.Cockpits.Manager.Sortie;
...
public class CockpitManagerNbDureeSortieEvolution extends BaseController {
	...
	private class GroupModel{
		...
		public GroupModel(IUser salarie, String direction, String mois){
		...
		public boolean equals(Object obj) {
			...
			return (mois.equals(otherGroup.mois) && direction.equals(otherGroup.direction) && salarie == otherGroup.salarie);
		}
	...
	protected static final Logger log = Logger.getLogger(CockpitManagerNbDureeSortieEvolution.class);
	...
	private JSONObject BuildData() {
		...
		{	
			...
			for (IWorkflowInstance sortie : demandesSorties)
			{
				...
				IUser demandeur = (IUser)sortie.getValue("Demandeur");
				...
				Date moisDate = (Date)sortie.getValue("dateSortie");
				Calendar x = Calendar.getInstance();
				x.setTime(moisDate);
				String mois = String.valueOf(x.get(Calendar.MONTH) + 1);
				GroupModel tmpGroupModel = new GroupModel(demandeur, direction, mois);
				if(!evolutionMap.containsKey(tmpGroupModel)){					
					for (int i = 1; i < 13; i++) {
						...
						GroupModel tmpG = new GroupModel(demandeur, direction, String.valueOf(i));

//						tmpGroupModel.mois = String.valueOf(i);
//						GroupModel tmpGroupModel = new GroupModel((IUser)sortie.getValue("Demandeur"), String.valueOf(i));
						evolutionMap.put(tmpG, tmp);
					}
				}
				
//				tmpGroupModel.mois = mois;
//				GroupModel tmpGroupModel = new GroupModel((IUser)sortie.getValue("Demandeur"), mois);
				JSONObject mapObject = evolutionMap.get(tmpGroupModel);
				mapObject.put("Nb sortie", (int)mapObject.get("Nb sortie") + 1);
				mapObject.put("Durée sortie", (int)mapObject.get("Durée sortie") + ((Number)sortie.getValue("dureeSortieMinutesReelle")).intValue());
