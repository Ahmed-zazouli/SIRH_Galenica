...
public class CockpitRHNbDureeAbsence extends BaseController {
	
	protected static final Logger log = Logger.getLogger(CockpitRHNbDureeAbsence.class);
		...
		try
		{
			...
			Collection<IWorkflowInstance> demandesConges =  getData();
				...
				for (IWorkflowInstance conge : demandesConges)
				{
					...
					String departement = (String)((IStorageResource) connectedUser.getExtendedAttributes().getValue("Departement")).getValue("sys_Title");
					...
					tmp.put("Salarié", ((IUser)conge.getValue("Demandeur")).getFullName());
					...
					if(conge.getValue("TypeDeConge") != null){
						if(conge.getValue("TypeDeConge").equals("CE")){
							tmp.put("Durée Absences", (float)conge.getValue("NombreDeJoursExceptionnelle"));
						}else{
							tmp.put("Durée Absences", (float)conge.getValue("NombreDeJoursDemandes"));
						}
					}
