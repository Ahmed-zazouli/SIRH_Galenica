...
public class ModuleAvanceSalaireKpi extends BaseController {

	protected static final Logger log = Logger.getLogger(ModuleAvanceSalaireKpi.class);
	...
	private JSONObject BuildData() {
		...
		try {
			...
			Collection<IWorkflowInstance> avances = getData();
			
			int demandeEnCours = 0;
			int demandeAcceptees = 0;
			int demandeRefusees = 0;
			
			for (IWorkflowInstance avance : avances) {
				if(avance.getValue("DocumentState").equals("Acceptée")){
					demandeAcceptees += 1;
				}else if(avance.getValue("DocumentState").equals("Refusée")){
					demandeRefusees += 1;
				}else{
					demandeEnCours += 1;
				}
			...
			tmp.put("kpi1_value", demandeAcceptees);
			tmp.put("kpi2_value", demandeEnCours);
			tmp.put("kpi3_value", demandeRefusees);
