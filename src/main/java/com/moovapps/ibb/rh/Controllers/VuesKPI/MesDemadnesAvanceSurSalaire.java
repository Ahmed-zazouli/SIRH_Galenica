...
public class MesDemadnesAvanceSurSalaire extends BaseController {
	
	protected static final Logger log = Logger.getLogger(MesDemadnesAvanceSurSalaire.class);
	...
	private JSONObject BuildData() {
		...
		try
		{
			...
			Collection<IWorkflowInstance> mesDemandesAvanceSurSalaire = getData();
			float demandesAvancesSurSalaireEnCours = 0;
			float demandesAvancesSurSalaireClotures = 0;
			float demandesAvancesSurSalaireRefuse = 0;
						
			Calendar c = Calendar.getInstance();
			SimpleDateFormat df = new SimpleDateFormat("dd/MM/yyyy");
			c.setTime(df.parse(df.format(c.getTime())));
		
			for (IWorkflowInstance demandeAvanceSalaire : mesDemandesAvanceSurSalaire) {
				if(demandeAvanceSalaire.getValue("DocumentState").equals("Refusée")){
					demandesAvancesSurSalaireRefuse++;
				} else if(demandeAvanceSalaire.getValue("DocumentState").equals("Acceptée")) {
					demandesAvancesSurSalaireClotures ++;
				} else{
					demandesAvancesSurSalaireEnCours++;
				}
			}
			
			tmp.put("Svalue1", demandesAvancesSurSalaireEnCours);
			tmp.put("Svalue2", demandesAvancesSurSalaireClotures);
			tmp.put("Svalue3", demandesAvancesSurSalaireRefuse); 
