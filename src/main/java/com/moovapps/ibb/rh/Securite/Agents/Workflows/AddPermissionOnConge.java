...
public class AddPermissionOnConge extends BaseAgent {

    @Override
    protected void execute() {
        try {
            ArrayList<IWorkflowInstance> conges = getConges();
            for (IWorkflowInstance conge : conges){

                try {
                    ISecurityController securityController = getSecuriteController(conge);
                    breakInheritance(securityController);
                    addManagementGroupes(securityController);
                    addSpecificPermissions(conge,securityController);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
    ...
    private ArrayList<IWorkflowInstance> getConges() {
