...
import com.moovapps.ibb.rh.Securite.Referentiels.Circuit;

import java.util.ArrayList;

public class AddPermissionOnCircuit extends BaseAgent {

    @Override
    protected void execute() {
        try{
            ArrayList<IStorageResource> circuits = getCircuits();
            if(circuits!=null && !circuits.isEmpty()){
                for(IStorageResource circuit : circuits){
                    ISecurityController securityController = getWorkflowModule().getSecurityController(circuit);
    ...
    ArrayList<IStorageResource> getCircuits(){
        ...
        try{
            ...
            IResourceDefinition definition = getWorkflowModule().getResourceDefinition(sysContext,catalog,"Categorie");
