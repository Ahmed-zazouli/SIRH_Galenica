...
public class AddPermissionOnRetard extends BaseAgent {

    @Override
    protected void execute() {
        try {
            ArrayList<IWorkflowInstance> retards = getRetards();
            for (IWorkflowInstance retard : retards){

                try {
                    ISecurityController securityController = getSecuriteController(retard);
                    breakInheritance(securityController);
                    addManagementGroupes(securityController);
                    addSpecificPermissions(retard,securityController);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
    ...
    private ArrayList<IWorkflowInstance> getRetards() {
        ...
        try
        {
            ...
            controller.addEqualsConstraint("sys_WorkflowContainer","GestionDeRetard");
