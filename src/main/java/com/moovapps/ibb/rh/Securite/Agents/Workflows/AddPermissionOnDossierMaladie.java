...
public class AddPermissionOnDossierMaladie extends BaseAgent {

    @Override
    protected void execute() {
        try {
            ArrayList<IWorkflowInstance> dossiersMaladie = getDossiersMaladie();
            for (IWorkflowInstance dossier : dossiersMaladie){

                try {
                    ISecurityController securityController = getSecuriteController(dossier);
                    breakInheritance(securityController);
                    addManagementGroupes(securityController);
                    addSpecificPermissions(dossier,securityController);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
    ...
    private ArrayList<IWorkflowInstance> getDossiersMaladie() {
        ...
        try
        {
            ...
            controller.addEqualsConstraint("sys_WorkflowContainer","DossiersMaladie");
