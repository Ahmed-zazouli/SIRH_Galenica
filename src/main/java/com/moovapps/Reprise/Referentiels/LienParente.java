...
public class LienParente extends BaseAgent {
    ...
    protected void execute() {
        try{
            ...
            List<String> columns = columnsHelper.lienParente;
            ...
            for (int rowIndex = 0; rowIndex <= sheet.getLastRowNum(); rowIndex++) {
                ...
                //Create pole storage
                IStorageResource lienParente = createStorage("DefaultOrganization","REFERENTIELCOMMUN","REFERENTIEL","LienParente");
                for(String colonne : columns){
                    if(colonne.startsWith("_")){
                        ...
                        method.invoke(this, lienParente,row,row.getCell(columns.indexOf(colonne)));
                    }
                }
                if(!isAnomalieFound){
                    lienParente.save(getWorkflowModule().getSysadminContext());
                }
                System.out.println();
            }
    ...
    public void _LienParente(IStorageResource instance, Row row, Cell cell){
        if(cell!=null && !cell.getCellType().equals(CellType.BLANK)){
            Object cellValue = null;
            if(cell.getCellType().equals(CellType.STRING)){
                cellValue = cell.getStringCellValue().trim();
    ...
    public void _NomPrenom(IStorageResource instance, Row row, Cell cell){
        if(cell!=null && !cell.getCellType().equals(CellType.BLANK)){
            Object cellValue = null;
            if(cell.getCellType().equals(CellType.STRING)){
                cellValue = cell.getStringCellValue().trim();
