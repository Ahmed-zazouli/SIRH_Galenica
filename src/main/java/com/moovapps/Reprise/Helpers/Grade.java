...
public class Grade {
    ...
    private List<String> grade = Arrays.asList("_Pole","_Societe","_Grade","_Ordre");
    ...
    public void CreateOrUpdateStorages(Sheet sheet, ArrayList<JSONObject> alertAnomalis){
        try{
            for (int rowIndex = 0; rowIndex <= sheet.getLastRowNum(); rowIndex++) {
                ...
                String gradeValue = gradeCell!=null?gradeCell.getStringCellValue().trim():"";

                if(poleValue.equals("") || societeValue.equals("") || gradeValue.equals("")){
                    //ANOMALIE
                    continue;
                }
                IStorageResource ressource = getOrCreateStorage(poleValue,societeValue,gradeValue);
                for(String colonne : grade){
                    if(colonne.startsWith("_")){
                        ...
                        method.invoke(this,alertAnomalis, ressource,row,row.getCell(grade.indexOf(colonne)));
                    }
                }
                if(!isAnomalieFound){
    ...
    public IStorageResource getOrCreateStorage(String pole , String societe,String grade) {
        try {
            ...
            IResourceDefinition definition = Modules.getWorkflowModule().getResourceDefinition(context,catalog,"Grade");
            ...
            controller.addEqualsConstraint("sys_Title",grade);
    ...
    public boolean _Grade(ArrayList<JSONObject> alertAnomalis, IStorageResource instance, Row row, Cell cell){
        try{
            if(cell!=null && !cell.getCellType().equals(CellType.BLANK)){
                Object cellValue = null;
                if(cell.getCellType().equals(CellType.STRING)){
                ...
                if(cellValue!=null && !((String) cellValue).trim().equals("")){
                    ...
                    addAnomalie(alertAnomalis,row.getSheet().getSheetName(),(row.getRowNum()+1)+"","Grade","Merci de vérifier le grade");
                    isAnomalieFound = true;
                }
            }else{
                addAnomalie(alertAnomalis,row.getSheet().getSheetName(),(row.getRowNum()+1)+"","Grade","Merci d'ajouter le grade");
                isAnomalieFound = true;
    ...
    public boolean _Ordre(ArrayList<JSONObject> alertAnomalis, IStorageResource instance, Row row, Cell cell){
        try{
            if(cell!=null && !cell.getCellType().equals(CellType.BLANK)){
                Object cellValue = null;
               if(cell.getCellType().equals(CellType.NUMERIC)){
                ...
                if(cellValue!=null){
                    instance.setValue("Ordre",cellValue);
                }else{
                    addAnomalie(alertAnomalis,row.getSheet().getSheetName(),(row.getRowNum()+1)+"","Ordre","Merci de vérifier l'ordre");
                     isAnomalieFound = true;
                }
            }else{
                addAnomalie(alertAnomalis,row.getSheet().getSheetName(),(row.getRowNum()+1)+"","Ordre","Merci d'ajouter l'ordre");
                  isAnomalieFound = true;
    ...
    public IStorageResource getOrCreateStorage(Row row) {
        try {
            ...
            IResourceDefinition definition = Modules.getWorkflowModule().getResourceDefinition(context,catalog,"Grade");
            IViewController controller = Modules.getWorkflowModule().getViewController(context,IResource.class);
            List<String> columns = Arrays.asList("_Pole","_Societe","*sys_Title");
