...
public class Activite {
    ...
    private List<String> activite = Arrays.asList("_Pole","_Societe","_Departement","_Activite");
    ...
    public void CreateOrUpdateStorages(Sheet sheet, ArrayList<JSONObject> alertAnomalis){
        try{
            for (int rowIndex = 0; rowIndex <= sheet.getLastRowNum(); rowIndex++) {
                ...
                if(poleValue.equals("") || societeValue.equals("") || departementValue.equals("") || activiteValue.equals("")){
                    //ANOMALIE
                    continue;
                }
                IStorageResource ressource = getOrCreateStorage(poleValue,societeValue,departementValue,activiteValue);
                for(String colonne : activite){
                    if(colonne.startsWith("_")){
                        ...
                        method.invoke(this,alertAnomalis, ressource,row,row.getCell(activite.indexOf(colonne)));
                    }
                }
                if(!isAnomalieFound){
    ...
    public IStorageResource getOrCreateStorage(String pole , String societe,String departement,String activite) {
        try {
            ...
            IResourceDefinition definition = Modules.getWorkflowModule().getResourceDefinition(context,catalog,"Activite");
            ...
            controller.addEqualsConstraint("sys_Title",activite);
    ...
    public IStorageResource getOrCreateStorage(Row row) {
        try {
            ...
            IResourceDefinition definition = Modules.getWorkflowModule().getResourceDefinition(context,catalog,"Activite");
            IViewController controller = Modules.getWorkflowModule().getViewController(context,IResource.class);
            List<String> columns = Arrays.asList("_Pole", "_Societe", "_Departement", "*sys_Title");
