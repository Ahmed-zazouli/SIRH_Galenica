...
public class Profil {
    ...
    private List<String> profil = Arrays.asList("_Pole","_Societe","_Profil");
    ...
    public void CreateOrUpdateStorages(Sheet sheet, ArrayList<JSONObject> alertAnomalis){
        try{
            for (int rowIndex = 0; rowIndex <= sheet.getLastRowNum(); rowIndex++) {
                ...
                Cell profilCell = row.getCell(2);
                String profilValue = profilCell!=null?profilCell.getStringCellValue().trim():"";

                if(poleValue.equals("") || societeValue.equals("") || profilValue.equals("")){
                    //ANOMALIE
                    continue;
                }
                IStorageResource ressource = getOrCreateStorage(poleValue,societeValue,profilValue);
                for(String colonne : profil){
                    if(colonne.startsWith("_")){
                        ...
                        method.invoke(this,alertAnomalis, ressource,row,row.getCell(profil.indexOf(colonne)));
                    }
                }
                if(!isAnomalieFound){
    ...
    public IStorageResource getOrCreateStorage(String pole , String societe,String profil) {
        try {
            ...
            controller.addEqualsConstraint("sys_Title",profil);
    ...
    public boolean _Profil(ArrayList<JSONObject> alertAnomalis, IStorageResource instance, Row row, Cell cell){
        try{
            if(cell!=null && !cell.getCellType().equals(CellType.BLANK)){
                Object cellValue = null;
                if(cell.getCellType().equals(CellType.STRING)){
                ...
                if(cellValue!=null && !((String) cellValue).trim().equals("")){
                    ...
                    addAnomalie(alertAnomalis,row.getSheet().getSheetName(),(row.getRowNum()+1)+"","Profil","Merci de vérifier le profil");
                    isAnomalieFound = true;
                }
            }else{
                addAnomalie(alertAnomalis,row.getSheet().getSheetName(),(row.getRowNum()+1)+"","Profil","Merci d'ajouter le profil");
                isAnomalieFound = true;
