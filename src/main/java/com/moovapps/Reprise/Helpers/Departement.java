...
public class Departement {
    ...
    private List<String> departement = Arrays.asList("_Pole","_Societe","_Departement");
    ...
    public void CreateOrUpdateStorages(Sheet sheet, ArrayList<JSONObject> alertAnomalis){
        try{
            for (int rowIndex = 0; rowIndex <= sheet.getLastRowNum(); rowIndex++) {
                ...
                if(poleValue.equals("") || societeValue.equals("") || departementValue.equals("")){
                    //ANOMALIE
                    continue;
                }
                IStorageResource ressource = getOrCreateStorage(poleValue,societeValue,departementValue);
                for(String colonne : departement){
                    if(colonne.startsWith("_")){
                        ...
                        method.invoke(this,alertAnomalis, ressource,row,row.getCell(departement.indexOf(colonne)));
                    }
                }
                if(!isAnomalieFound){
    ...
    public IStorageResource getOrCreateStorage(String pole , String societe,String departement) {
        try {
            ...
            controller.addEqualsConstraint("sys_Title",departement);
