...
public class MissionCollaborateur {
    ClassListHelper helper = new ClassListHelper();
    boolean isAnomalieFound = false;
    private List<String> missionCollaborateur = Arrays.asList("_Collaborateur","_Mission","_DateAffectation","_Details");
    ...
    public void CreateOrUpdateStorages(Sheet sheet, ArrayList<JSONObject> alertAnomalis){
        try{
            for (int rowIndex = 0; rowIndex <= sheet.getLastRowNum(); rowIndex++) {
                ...
                Cell missionCell = row.getCell(1);
                String missionValue = missionCell!=null?missionCell.getStringCellValue().trim():"";



                if(collaborateurCINValue.equals("") || missionValue.equals("")){
                    //ANOMALIE
                    continue;
                }
                IStorageResource ressource = getOrCreateStorage(collaborateurCINValue,missionValue);
                for(String colonne : missionCollaborateur){
                    if(colonne.startsWith("_")){
                        ...
                        method.invoke(this,alertAnomalis, ressource,row,row.getCell(missionCollaborateur.indexOf(colonne)));
                    }
                }
                if(!isAnomalieFound){
    ...
    public IStorageResource getOrCreateStorage(String cin , String mission) {
        try {
            ...
            controller.addEqualsConstraint("sys_Title",mission);
    ...
    public boolean _DateAffectation(ArrayList<JSONObject> alertAnomalis, IStorageResource instance, Row row, Cell cell){
        try{
            if(cell!=null && !cell.getCellType().equals(CellType.BLANK)){
                Object cellValue = null;
                if(cell.getCellType().equals(CellType.STRING)){
                ...
                if(cellValue!=null){
                    instance.setValue("DateAffectation",cellValue);
                }else{
