...
public class Metier {
    ...
    private List<String> metier = Arrays.asList("_Pole","_Societe","_Metier");
    ...
    public void CreateOrUpdateStorages(Sheet sheet, ArrayList<JSONObject> alertAnomalis){
        try{
            for (int rowIndex = 0; rowIndex <= sheet.getLastRowNum(); rowIndex++) {
                ...
                Cell metierCell = row.getCell(2);
                String metierValue = metierCell!=null?metierCell.getStringCellValue().trim():"";

                if(poleValue.equals("") || societeValue.equals("") || metierValue.equals("")){
                    //ANOMALIE
                    continue;
                }
                IStorageResource ressource = getOrCreateStorage(poleValue,societeValue,metierValue);
                for(String colonne : metier){
                    if(colonne.startsWith("_")){
                        ...
                        method.invoke(this,alertAnomalis, ressource,row,row.getCell(metier.indexOf(colonne)));
                    }
                }
                if(!isAnomalieFound){
    ...
    public IStorageResource getOrCreateStorage(String pole , String societe,String metier) {
        try {
            ...
            IResourceDefinition definition = Modules.getWorkflowModule().getResourceDefinition(context,catalog,"Metier");
            ...
            controller.addEqualsConstraint("sys_Title",metier);
    ...
    public boolean _Metier(ArrayList<JSONObject> alertAnomalis, IStorageResource instance, Row row, Cell cell){
        try{
            if(cell!=null && !cell.getCellType().equals(CellType.BLANK)){
                Object cellValue = null;
                if(cell.getCellType().equals(CellType.STRING)){
                ...
                if(cellValue!=null && !((String) cellValue).trim().equals("")){
                    ...
                    addAnomalie(alertAnomalis,row.getSheet().getSheetName(),(row.getRowNum()+1)+"","Métier","Merci de vérifier le métier");
                    isAnomalieFound = true;
                }
            }else{
                addAnomalie(alertAnomalis,row.getSheet().getSheetName(),(row.getRowNum()+1)+"","Métier","Merci d'ajouter le métier");
                isAnomalieFound = true;
