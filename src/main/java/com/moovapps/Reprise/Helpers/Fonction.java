...
public class Fonction {
    ...
    private List<String> fonction = Arrays.asList("_Pole","_Societe","_Fonction","_Description");
            ...
            for (int rowIndex = 0; rowIndex <= sheet.getLastRowNum(); rowIndex++) {
                ...
                for(String colonne : fonction){
                    if(colonne.startsWith("_")){
                        ...
                        method.invoke(this,alertAnomalis, ressource,row,row.getCell(fonction.indexOf(colonne)));
                    }
                }
                if(!isAnomalieFound){
    ...
    public IStorageResource getOrCreateStorage(String pole , String societe,String fonction) {
        try {
            ...
            controller.addEqualsConstraint("sys_Title",fonction);
    ...
    public boolean _Description(ArrayList<JSONObject> alertAnomalis, IStorageResource instance, Row row, Cell cell){
        try{
            if(cell!=null && !cell.getCellType().equals(CellType.BLANK)){
                Object cellValue = null;
                if(cell.getCellType().equals(CellType.STRING)){
                ...
                if(cellValue!=null && !((String) cellValue).trim().equals("")){
                    instance.setValue("PosteDescriptive",cellValue);
                }else{
                    addAlerte(alertAnomalis,row.getSheet().getSheetName(),(row.getRowNum()+1)+"","Description","Merci de vérifier la description");
                   // isAnomalieFound = true;
                }
            }else{
                addAlerte(alertAnomalis,row.getSheet().getSheetName(),(row.getRowNum()+1)+"","Description","Merci d'ajouter la description");
             //   isAnomalieFound = true;
