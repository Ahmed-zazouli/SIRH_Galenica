...
public class FamilleCompetence {
    ClassListHelper helper = new ClassListHelper();
    boolean isAnomalieFound = false;
    private List<String> familleCompetence = Arrays.asList("_Domaine","_FamilleCompetence");
    ...
    public void CreateOrUpdateStorages(Sheet sheet, ArrayList<JSONObject> alertAnomalis){
        try{
            for (int rowIndex = 0; rowIndex <= sheet.getLastRowNum(); rowIndex++) {
                ...
                if(domaineValue.equals("") || familleCompetenceValue.equals("")){
                    //ANOMALIE
                    continue;
                }
                IStorageResource ressource = getOrCreateStorage(domaineValue,familleCompetenceValue);
                for(String colonne : familleCompetence){
                    if(colonne.startsWith("_")){
                        ...
                        method.invoke(this,alertAnomalis, ressource,row,row.getCell(familleCompetence.indexOf(colonne)));
                    }
                }
                if(!isAnomalieFound){
    ...
    public IStorageResource getOrCreateStorage(String domaine , String familleCompetence) {
        try {
            ...
            IResourceDefinition definition = Modules.getWorkflowModule().getResourceDefinition(context,catalog,"FamilleCompetence");
            ...
            controller.addEqualsConstraint("sys_Title",familleCompetence);
    ...
    public IStorageResource getOrCreateStorage(Row row) {
        try {
            ...
            IResourceDefinition definition = Modules.getWorkflowModule().getResourceDefinition(context,catalog,"FamilleCompetence");
            IViewController controller = Modules.getWorkflowModule().getViewController(context,IResource.class);
            List<String> columns = Arrays.asList("_Domaine","*sys_Title");
