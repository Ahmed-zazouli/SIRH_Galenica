...
public class VueCollaborateursParProfil extends BaseViewExtension implements IRequestGenerator {

    @Override
    public String onPrepareSQL(String sqlQuery) {
        //sqlQuery = sqlQuery.replace("( r.Profil =","( r.Profil in" );
        sqlQuery = sqlQuery.replace("( ? =", "( ?=5 and r.Profil in");
        // SELECT COUNT(*) FROM r_stoprofil r WHERE ((((( ? =( ? , ? )) ) ) ) )
        //SELECT COUNT(*) FROM r_stodivision r WHERE (((((  ?=5 and r.Departement in ( ? , ? ) or r.Division in ( ? , ? )) ) ) ) )
        //sqlQuery = sqlQuery.replace("( r.Division =", "(r.Division in");
