...
public class PlanActionParOrigineParEfficacite extends BaseController{
protected static final Logger log = Logger.getLogger(PlanActionParOrigineParEfficacite.class);
	...
	private JSONObject chercheAdresse(String participation)
	{
		...
		try
		{
			...
			if(planActions!=null && !planActions.isEmpty()){
				...
				if(iWorkflowInstance.getValue("Efficacite")!=null){
					if(iWorkflowInstance.getValue("Efficacite").equals("Oui")){
						data.put("Efficacite", "Efficace");
					}else{
						data.put("Efficacite", " Non efficace");
					}
				}
	...
	private Collection<IWorkflowInstance> getActionsFiltrer(){
		...
		final String[] lien = { "uril://vdoc/workflowContainerView/DefaultOrganization/WIZEORGA/Processus:0/PlanDAction/BarChartPlanParEfficacite" };
		try {
			String[] array;
