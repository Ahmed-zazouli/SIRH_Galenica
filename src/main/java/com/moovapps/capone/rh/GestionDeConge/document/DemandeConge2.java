...
public class DemandeConge2 extends BaseDocumentExtension {
...
//CongesSansSoldeEnCoursDeTraitement
    ...
    public void calculateCongesPayes() {
        ...
        if (dateDebutConge != null && dateFinConge != null) {
            ...
            nbrJoursDemande -= calculateTrancheMinusValue(document,dateDebutConge, dateFinConge, nbrJoursToAdd,(int)calculResult.get("diffJoursOuvrablesJoursOuvres"),(boolean)calculResult.get("injoursOuvrablesNotjoursOuvres"));
    ...
    private float calculateTrancheMinusValue(IWorkflowInstance document, Date dateDebutConge, Date dateFinConge, int nbrJoursToAdd , int diffJoursOuvrablesJoursOuvres , boolean injoursOuvrablesNotjoursOuvres) {
        float heureDebutFinMinus = 0;
        Calendar endCalendar = Calendar.getInstance();
        endCalendar.setTime(dateFinConge);

        String debut = (String) document.getValue("DebutConge");
        String fin = (String) document.getValue("FinConge");
                ...
                if(dateDebutConge.before(dateDebutCurrentMonth)){//back here
                    if (conge.getValue("TypeCongeExceptionnelle") != null){
                        if (societe.getValue("CongeSpeciaux").equals("Ouvrables")) {
                            ...
                            float nbrJoursDemande = (Float) calculResult.get("nbrJoursDemande");
                            int nbrJoursToAdd = (int) calculResult.get("nbrJoursToAdd");
                            nbrJoursDemande -= calculateTrancheMinusValue(conge,dateDebutCurrentMonth, (Date) conge.getValue("DateDeFin"), nbrJoursToAdd,0,false);
