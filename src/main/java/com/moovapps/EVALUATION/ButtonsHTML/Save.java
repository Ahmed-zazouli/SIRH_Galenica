...
public class Save {
    ...
    public void generer(IWorkflowInstance workflowInstance, IResourceController resourceController){
        try {
            ...
            ArrayList<NotesModel> data =  buildFamilleArray(competences);
            addSousFamilleToArray(competences,data);
            addCompetenceToArray(competences,data);





            rowIndex = fillCompetence2(data,workbook,sheet,rowIndex,workflowInstance);
            rowIndex++;
            rowIndex++;
...
*/
    ...
    int fillCompetenceCHATGPT(ArrayList<NotesModel> jsonArray, Workbook workbook, Sheet sheet, int rowIndex, IWorkflowInstance instance) {
        for (NotesModel familleObject : jsonArray) {
            if (familleObject.getParentID().isEmpty()) {
                ...
                for (NotesModel sousFamilleObject : jsonArray) {
                    if (sousFamilleObject.getParentID().equals(familleObject.getId())) {
                        rowIndex = processSousFamille(workbook, sheet, rowIndex, sousFamilleObject);
                        ArrayList<Float> competenceNotes = processCompetence(workbook, sheet, rowIndex, jsonArray, sousFamilleObject.getId());
    ...
    private ArrayList<Float> processCompetence(Workbook workbook, Sheet sheet, int rowIndex, ArrayList<NotesModel> jsonArray, String sousFamilleId) {
        rowIndex++;
        ArrayList<Float> competenceNotes = new ArrayList<>();
        for (NotesModel competenceObject : jsonArray) {
            if (competenceObject.getParentID().equals(sousFamilleId)) {
