...
public class CoursProvider extends ResourceViewProvider {
    public CoursProvider(INavigateContext context, CtlAbstractView view) {
        super(context, view);
    }
    ...
    public void loadModel()
    {
        ...
        if ( viewDefinition != null && /*StringUtils.isNotEmpty( this.xmlViewElement.getAttribute( CONTEXT_PARAMETER_COLUMN ) ) &&*/ this.context.getParameterObject( IProvider.CURRENT_DOCUMENT ) != null )
        {
            ...
            try
            {
                ...
                fg.addFilter( new PropertyFilter( "Participant2", Operator.EQUALS, coreDocument.getResource().getValue("Participant2") ) );
                fg.addFilter( new PropertyFilter( "IDSession", Operator.EQUALS, coreDocument.getResource().getValue("IDSession") ) );
