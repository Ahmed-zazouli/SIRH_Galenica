...
public class SuiviProgrammeQuestionnaireFroidProvider extends ResourceViewProvider {
    public SuiviProgrammeQuestionnaireFroidProvider(INavigateContext context, CtlAbstractView view) {
        super(context, view);
    }
    ...
    public void loadModel()
    {
        ...
        if ( viewDefinition != null && /*StringUtils.isNotEmpty( this.xmlViewElement.getAttribute( CONTEXT_PARAMETER_COLUMN ) ) &&*/ this.context.getParameterObject( IProvider.CURRENT_DOCUMENT ) != null )
        {
            ...
            if ( navigationPartName == null ) // Le pb est qu'en delete on reste sr lua m�me part => On le r�cup�re donc uniquement � la premi�re init
            {
            ...
            try
            {
                ...
                fg.addFilter( new PropertyFilter( "IDProgrammeQuestionFroid", Operator.EQUALS, coreDocument.getResource().getValue("IDProgrammeQuestionFroid") != null ? coreDocument.getResource().getValue("IDProgrammeQuestionFroid") : "Empty" ) );
                fg.addFilter( new PropertyFilter( "IDSeanceParticipant", Operator.EQUALS, coreDocument.getResource().getValue("IDSeanceParticipant")) );
//                fg.addFilter( new PropertyFilter( filterColumn, Operator.EQUALS, value ) );
                ...
                if ( createButton != null && value != null && rd != null ) // On ne g�re la cr�ation que pour le data universe pour l'instant
                {
                    ...
                    coreDocument = (CoreDocument)this.context.getParameterObject( IProvider.CURRENT_DOCUMENT );
