...
public class VDocValuesHelperForBookmarks
{
	...
	public static void setType(Object resourceValue, Bookmark bookmark, DocumentBuilder builder, IWorkflowModule workflowModule) throws Exception
	{
		...
		else
		{
			...
			}else if(resourceValue instanceof Boolean){
				VDocValuesHelperForBookmarks.setBooleanType(resourceValue, bookmark);
	...
	public static void setBooleanType(Object value, Bookmark bookmark) throws Exception
	{
		bookmark.setText(value!=null?value.toString():"");
		if(value!=null){
			if(value instanceof Boolean){
				if((Boolean) value == true){
					bookmark.setText("Actif");
				}else if((Boolean) value == false){
					bookmark.setText("Inactif");
				}
			}
		}
	...
	public static void setNumberType(Object numberValue, Bookmark bookmark, IWorkflowModule workflowModule) throws Exception
	{
		...
		if(! (numberValue instanceof BigDecimal)){
			if(((Float) numberValue) % 1 == 0){
				bookmark.setText(((Number) numberValue).intValue()+"");
			}else{
				bookmark.setText(decimalFormat.format(numberValue));
			}
		}else{
			if(((BigDecimal) numberValue).remainder(BigDecimal.ONE)  == BigDecimal.ZERO){
				bookmark.setText(((Number) numberValue).intValue()+"");
			}else{
				bookmark.setText(decimalFormat.format(numberValue));
			}
