...
public class CandidatureDuSite extends BaseViewExtension 
{
	...
	public void onPrepareItem(ViewItem iViewItem) 
	{
		try 
		{
			...
			CtlButton button = new CtlButton("POSTULER", new CtlText("POSTULER"));
	...
	protected ActionListener Candidature = new ActionListener() 
	{
		...
		public void onClick(ActionEvent paramActionEvent) {
			try 
			{
				if(getDirectoryModule().getLoggedOnUser().isAnonymous()){
					String lien2 = Turbine.getServerScheme().concat("://").concat(Turbine.getServerName()).concat(":").concat(Turbine.getServerPort()).concat(Turbine.getContextPath().concat( "/easysite/workplace/welcome/"));
					ExternalScreen externalScreen2 = new ExternalScreen(lien2);
					Navigator.getNavigator().pushScreen(externalScreen2);
				}
				...
				instance = getWorkflowModule().createWorkflowInstance(context, workflow, null);
...
//						.concat(Turbine.getContextPath().concat(instance.getURI()));
				String lien = Turbine.getServerScheme().concat("://").concat(Turbine.getServerName()).concat(":").concat(Turbine.getServerPort()).concat(Turbine.getContextPath().concat(instance.getURI().replace("/easysite/plugin:sys.process/", "/easysite/workplace/candidature/candidature/edit-document/")));
				...
				//Navigator.getNavigator().cleanChildrenNavigators(externalScreen);
