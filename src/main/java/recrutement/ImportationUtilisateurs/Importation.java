...
public class Importation {
    ...
    HashMap<IUser, List<Object>> N1Map = new HashMap<>();
    HashMap<IUser, List<Object>> EvaluatorMap = new HashMap<>();
    IStorageResource responsableEtablissement = null;
    IStorageResource evaluateurEtablissement = null;
    ...
    public void importer(IWorkflowInstance instance, IResourceController resourceController){
        {
            try{

                this.instance = instance;
                ArrayList<IAttachment> files = (ArrayList<IAttachment>) instance.getValue("UtilisateursFile");
                if(files!=null && !files.isEmpty()){
                    if(files.size()>1){
                        //u cant put more than one file
                        //Return false
                        resourceController.alert("Veuillez renseigner un seul fichier à importer");
                    ...
                    if(!excelAnomalies.isEmpty()){
                        ...
                        if(!filePath.equals("")){
                            ...
                            //add piece joint
                            IAttachment attachement = directoryModule.createAttachment(workflowModule.getSysadminContext(), new File(filePath));
                            ArrayList<IAttachment>  attachements = new ArrayList();
                            attachements.add(attachement);
                            instance.setValue("RapportDAnomalie",attachements);
                            instance.save(workflowModule.getSysadminContext());
                        }

                    }
                    if(anomalieFileCreated){
                        resourceController.alert("Anomalies found");
                    }
                }else{
                    // veuiller rensengner un fichier
                    //Return false
                    resourceController.alert("Veuillez renseigner un fichier à importer");
    ...
    public void setN1() {
        for (Map.Entry<IUser, List<Object>> entry : N1Map.entrySet()) {
            IUser user = entry.getKey();
            List N1Info = entry.getValue();
            if(user==null){
                continue;
            }
            if (N1Info != null) {
                String matricule = (String) N1Info.get(0);
                IStorageResource responsableEtablissement = (IStorageResource) N1Info.get(1);
                int index = (int) N1Info.get(2);
                IUser N1 = getUserByEtablissementAndMat(matricule,responsableEtablissement);
                if (N1 != null) {
                    ...
                    user.save(workflowModule.getSysadminContext());
                    userRef.save(workflowModule.getSysadminContext());
                } else {
    ...
    public void setEvaluator() {
        for (Map.Entry<IUser, List<Object>> entry : EvaluatorMap.entrySet()) {
            IUser user = entry.getKey();
            List EvaluatorInfo = entry.getValue();
            if(user==null){
                continue;
            }
            if (EvaluatorInfo != null) {
                String matricule = (String) EvaluatorInfo.get(0);
                IStorageResource evaluatorEtablissement = (IStorageResource) EvaluatorInfo.get(1);
                int index = (int) EvaluatorInfo.get(2);
                IUser Evaluator = getUserByEtablissementAndMat(matricule,evaluatorEtablissement);
                if (Evaluator != null) {
                    ...
                    addAnomalie(AnnuaireFromExcel.AnomalieTypes.A,"Merci de vérifier l'Evaluateur avec le matricule "+matricule,index,"Evaluateur");
                }
            }
            // ...
        }
        ...
        try {
            ...
            try {
                ...
                while (annuaireIterator.hasNext()) {
                    ...
                    isUpdate = false;
                    responsableEtablissement = null;
                    evaluateurEtablissement = null;
                ...
                if(etablissementOrganization!=null){
                    if(!PasswordHelper.checkPasswordStrength(password, "")){
                        ...
                        try {
                            ...
                            finalUser = directoryModule.createUser(sysContext, login, password, etablissementOrganization);
                        }catch (Exception e){
                            if( e.getMessage()!=null && e.getMessage().equals("L'identifiant existe déja")){
                                addAnomalie(AnnuaireFromExcel.AnomalieTypes.B, "L'identifiant exist deja",rowIndex,"Login");
                                isAnomalieFound = true;
    ...
    IOrganization getOrganizationByEtablissement(String etablissement) {
        try {
            ...
            IResourceDefinition definition = workflowModule.getResourceDefinition(context, catalog, "Societe");
    ...
    public IStorageResource getOrCreateFicheCollaborateurByUser(IUser salarie){
        try{
            IContext context = workflowModule.getSysadminContext();
            IProject project = projectModule.getProject(context,"REFERENTIELCOMMUN",directoryModule.getOrganization(context, "DefaultOrganization"));
            ICatalog catalog = workflowModule.getCatalog(context,"REFERENTIEL", 4, project);
            IResourceDefinition resourceDefinition = workflowModule.getResourceDefinition(context, catalog, "FicheCollaborateur");
            IViewController controller = workflowModule.getViewController(context, IResource.class);
            ...
            if(controller.evaluate(resourceDefinition).size()>0){
                ...
                return workflowModule.createStorageResource(context,resourceDefinition,null,null);
    ...
    void  setUserFileds(IStorageResource ficheCollaborateur,IUser user){
        ...
        ficheCollaborateur.setValue("Evaluateur",user.getExtendedAttributes().getValue("Evaluateur"));
        ...
        ficheCollaborateur.save(workflowModule.getSysadminContext());
    }
...
//		return (int)difference_In_Years;
    ...
    public void _EtablissementN1(IUser user, Cell cell, int rowIndex) {
        if (cell != null && cell.getCellType() != CellType.STRING) {
            ...
            if (cellValue != null) {
                ...
                if (etablissement == null) {
                    addAnomalie(AnnuaireFromExcel.AnomalieTypes.A,"Merci de vérifier l'etablissement "+ cellValue,rowIndex,"Etablissement Responsable");
                }else{
                    responsableEtablissement = etablissement;
                }
            }else{
                addAnomalie(AnnuaireFromExcel.AnomalieTypes.A,"Merci de vérifier l'etablissement",rowIndex,"Etablissement Responsable");
            }

        }else{
            addAnomalie(AnnuaireFromExcel.AnomalieTypes.A,"Merci d'ajouter l'etablissement",rowIndex,"Etablissement Responsable");
    ...
    public void _N1(IUser user, Cell cell, int rowIndex) {
        if (cell != null && cell.getCellType() != CellType.STRING) {
            ...
            if(cellValue!=null){
                    String N1Mat = (String) cellValue;
                    IUser N1 = getUserByEtablissementAndMat(N1Mat,responsableEtablissement);
                    if (N1 != null) {
                        user.setHierarchicalManager(N1);
                    } else {
                        N1Map.put(user, new ArrayList<>(Arrays.asList(cellValue,responsableEtablissement,rowIndex)));
                    }
            }
        }else{
    ...
    public void _EtablissementEvaluator(IUser user, Cell cell, int rowIndex) {
        if (cell != null && cell.getCellType() != CellType.STRING) {
            Object cellValue = null;
            if (cell.getCellType() == CellType.NUMERIC) {
                cellValue = cell.getNumericCellValue();
            ...
            if (cellValue != null) {
                IStorageResource etablissement = getEtablissement(((String) cellValue).toLowerCase());
                if (etablissement == null) {
                    addAnomalie(AnnuaireFromExcel.AnomalieTypes.A,"Merci de vérifier l'etablissement "+ cellValue,rowIndex,"Etablissement evaluateur");
                }else{
                    evaluateurEtablissement = etablissement;
                }
            }else{
                addAnomalie(AnnuaireFromExcel.AnomalieTypes.A,"Merci de vérifier l'etablissement",rowIndex,"Etablissement evaluateur");
            }

        }else{
            addAnomalie(AnnuaireFromExcel.AnomalieTypes.A,"Merci d'ajouter l'etablissement",rowIndex,"Etablissement evaluateur");
    ...
    public void _Evaluator(IUser user, Cell cell, int rowIndex) {
        if (cell != null && cell.getCellType() != CellType.STRING) {
            ...
            if(cellValue!=null){
                    String evaluatorMat = (String) cellValue;
                    IUser Evaluator = getUserByEtablissementAndMat(evaluatorMat,evaluateurEtablissement);
                    if (Evaluator != null) {
                        user.getExtendedAttributes().setValue ("Evaluateur",Evaluator);
                    } else {
                        EvaluatorMap.put(user, new ArrayList<>(Arrays.asList(cellValue,evaluateurEtablissement,rowIndex)));
    ...
    public ArrayList<IGroup> getGroupes(String[] grouupes, int rowIndex) {
        try {
            IContext sysContext = workflowModule.getSysadminContext();
            IOrganization IEGOrga = directoryModule.getOrganization(sysContext, "IEGGroupes");
            ...
            for (String groupe : grouupes) {
                if (groupe != null && !groupe.equals("")) {
                    if (groupe.equals("Salarie")) {
                        groupes.add(directoryModule.getGroup(sysContext, IEGOrga, "Salarie"));
                    } else if (groupe.equals("RH")) {
                        groupes.add(directoryModule.getGroup(sysContext, IEGOrga, "RH"));
                    } else if (groupe.equals("Direction")) {
                        groupes.add(directoryModule.getGroup(sysContext, IEGOrga, "GroupePersonnelDeDirection"));
                    } else if (groupe.equals("Administratif")) {
                        groupes.add(directoryModule.getGroup(sysContext, IEGOrga, "Collaborateurs_Eval"));
                    } else if (groupe.equals("Manager")) {
                        groupes.add(directoryModule.getGroup(sysContext, IEGOrga, "Manager"));
                    } else if (groupe.equals("Evaluateur")) {
                        groupes.add(directoryModule.getGroup(sysContext, IEGOrga, "Evaluateurs"));
                    } else if (groupe.equals("Enseignant")) {
                        groupes.add(directoryModule.getGroup(sysContext, IEGOrga, "GroupeEnseignants"));
                    } else if(groupe.equals("IEGGroupe")){
                        groupes.add(directoryModule.getGroup(sysContext, IEGOrga, "ADMIN"));
                    } else if(groupe.equals("DRH")){
                        groupes.add(directoryModule.getGroup(sysContext, IEGOrga, "DRH"));
                    }else if(groupe.equals("Consultation evaluation")){
                        groupes.add(directoryModule.getGroup(sysContext, IEGOrga, "ConsultationEvaluation"));
                    }else if(groupe.equals("Conseiller pédagogique")){
                        groupes.add(directoryModule.getGroup(sysContext, IEGOrga, "SupportPedagogique"));
                    }else if(groupe.equals("Documentaliste")){
                        groupes.add(directoryModule.getGroup(sysContext, IEGOrga, "Documentaliste"));
                    }else if(groupe.equals("Conseiller Principal d’Education")){
                        groupes.add(directoryModule.getGroup(sysContext, IEGOrga, "CPE"));
                    }
                    else {
            ...
            groupes.add(directoryModule.getGroup(sysContext, IEGOrga, "NEWS_ANIMATION"));
            return groupes;
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        if (cell != null && cell.getCellType() != CellType.STRING) {
            ...
            if (cellValue != null) {
                ...
                if (userGroupes == null) {
                    ...
                    try {
                        IContext context = workflowModule.getSysadminContext();
                        IOrganization organization = directoryModule.getOrganization(context, "IEGGroupes");
                        IGroup GroupeEnseignants = directoryModule.getGroup(context, organization, "GroupeEnseignants");
                        IGroup GroupeAdministratif = directoryModule.getGroup(context, organization, "Collaborateurs_Eval");
                        IGroup GroupeDirection = directoryModule.getGroup(context, organization, "GroupePersonnelDeDirection");
                        int sum = 0;
    ...
    private IResourceDefinition getContratNameDefinition() {
        ...
        try {
            IContext context = workflowModule.getSysadminContext();
            IOrganization organization = directoryModule.getOrganization(context, "DefaultOrganization");
            IProject project = projectModule.getProject(context, "Recrutement", organization);
//			IContext context = getWorkflowModule().getLoggedOnUserContext();
    ...
    IUser getUserByEtablissementAndMat(String mat,IStorageResource etablissement){
        IUser user = null;
        try {
            IContext context = workflowModule.getSysadminContext();
            IViewController controller = workflowModule.getViewController(context, IResource.class);
